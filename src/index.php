...
$page = isset($_GET['page']) ? intval($_GET['page']) : 1; // on vérifie avec isset si l'url comprend bien un numéro de page, puis via une expression ternaire on applique par défault un numéro de page a 1 ou on recupère le numéro de la page avec la supervariable $get pour la stocker dans la variable $page 
        ...
        <nav aria-label="Page navigation">
            <ul class="list-style-none flex">
                <?php for ($i = 1; $i <= $totalPages; $i++): // boucle for a l'ancienne pour iterer dans le nombre de page total?>
                    <li><a href="?page=<?= htmlspecialchars($i)?>">
                            <?= htmlspecialchars($i) ?>
